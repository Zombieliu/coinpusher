/*
 * @Author: dgflash
 * @Date: 2022-05-05 17:20:19
 * @LastEditors: dgflash
 * @LastEditTime: 2022-07-12 18:31:30
 */
import { Db, MongoClient } from "mongodb";
import { Config } from "../config/Config";

/** 数据表名 */
export enum DbCollectionName {
    /** 自增量记录表 */
    counters = "counters",
    /** 用户表 */
    user = "user",
}

export class MongoDB {
    static db: Db;

    /** 实始化 mongodb 数据库 */
    static async init() {
        // 优先使用环境变量 MONGO_URI，如果没有则使用 Config.mongodb
        const url = process.env.MONGO_URI || `mongodb://${Config.mongodb}/`;
        const dbName = process.env.DB_NAME || "coinpusher_game";
        const client = await new MongoClient(url).connect();        // 连接数据库
        this.db = client.db(dbName);                                // 打开数据库，如果不存在就创建一个

        // 初始化数据表
        for (var name in DbCollectionName) {
            if (!await this.collectionExist(name)) {
                await this.db.createCollection(name);
            }
        }
    }

    /** 数据表是否存在 */
    static async collectionExist(name: string): Promise<boolean> {
        try {
            const collinfo = await this.db.listCollections({ name }).next();
            return collinfo !== null;
        } catch (err) {
            return false;
        }
    }

    /**
     * 获取指定数据表的自增量
     * @param name
     * @returns
     */
    static async getNextSequenceValue(name: string): Promise<number> {
        const conters = this.db.collection(DbCollectionName.counters);

        const result = await conters.findOneAndUpdate(
            { key: name },
            { $inc: { value: 1 } },
            { returnDocument: 'after', upsert: false }
        );

        // 没有指定配置表自增量记录
        if (!result) {
            await conters.insertOne({
                key: name,
                value: 1
            });
            return await this.getNextSequenceValue(name);
        }

        return result.value;
    }
}